      ...
      <div className="relative w-full max-w-5xl bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden my-8">
        ...
        <div className="flex items-center justify-between p-6 border-b border-slate-800 bg-slate-950/60">
          <div className="flex items-center space-x-3">
            <div className="p-2.5 bg-gradient-to-tr from-indigo-600 to-purple-600 rounded-xl text-white shadow-lg shadow-indigo-600/30 animate-pulse">
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-16 space-y-4">
              ...
              <div className="text-center space-y-1">
                <p className="text-slate-200 font-medium">Autonomous Agent is crawling web & company 5-year histories...</p>
            ...
            <>
              ...
              <div className="p-5 rounded-xl bg-gradient-to-r from-indigo-950/60 to-purple-950/60 border border-indigo-500/30 flex items-start space-x-4">
                <Activity className="w-6 h-6 text-indigo-400 shrink-0 mt-0.5" />
                <div className="space-y-1">
                  <div className="flex items-center space-x-2">
                    <span className="text-xs font-bold uppercase tracking-wider text-indigo-400">Agent Market Verdict</span>
                  ...
                  <p className="text-slate-200 font-medium text-sm leading-relaxed">{report.market_verdict}</p>
                </div>
              </div>
                    ...
                    {report.bullish_profit_candidates.map((stock: AgentStockInsight) => (
                      ...
                      >
                        <div className="flex justify-between items-start">
                          <div>
                            <h4 className="font-bold text-slate-100 text-base group-hover:text-emerald-400 transition-colors flex items-center space-x-1.5">
                          ...
                          <div className="text-right">
                            <span className="text-[10px] text-slate-500 block">Probability Score</span>
                        ...
                        <div className="grid grid-cols-3 gap-2 bg-slate-900/60 p-2.5 rounded-lg text-[11px] font-mono text-center">
                          <div>
                            <span className="text-slate-500 block text-[9px]">5Y CAGR</span>
                            <span className="font-bold text-emerald-400">+{stock.five_year_cagr}%</span>
                          </div>
                          <div>
                            <span className="text-slate-500 block text-[9px]">5Y High</span>
                            <span className="font-bold text-slate-200">₹{stock.five_year_high}</span>
                          </div>
                          <div>
                            <span className="text-slate-500 block text-[9px]">5Y Low</span>
                            <span className="font-bold text-slate-200">₹{stock.five_year_low}</span>
                          </div>
                        </div>

                        <p className="text-xs text-slate-300 leading-relaxed font-sans">{stock.agent_reasoning}</p>

                        <div className="pt-2 border-t border-slate-900 text-[11px]">
                          <span className="text-emerald-400 font-semibold">Key Advantage: </span>
                          <span className="text-slate-400">{stock.strengths[0]}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {report.bearish_loss_risk_candidates.map((stock: AgentStockInsight) => (
                      <div
                        key={stock.symbol}
                        onClick={() => { onClose(); onSelectStock(stock.symbol); }}
                        className="p-5 rounded-xl bg-slate-950 border border-slate-800 hover:border-rose-500/40 transition-all cursor-pointer space-y-3 group"
                      >
                        <div className="flex justify-between items-start">
                          <div>
                            <h4 className="font-bold text-slate-100 text-base group-hover:text-rose-400 transition-colors flex items-center space-x-1.5">
                              <span>{stock.symbol}</span>
                          ...
                          <div className="text-right">
                            <span className="text-[10px] text-slate-500 block">Probability Score</span>
                            <span className="text-lg font-bold font-mono text-rose-400">{stock.probability_score}/100</span>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-3 gap-2 bg-slate-900/60 p-2.5 rounded-lg text-[11px] font-mono text-center">
                          <div>
                            <span className="text-slate-500 block text-[9px]">5Y CAGR</span>
                            <span className="font-bold text-rose-400">{stock.five_year_cagr}%</span>
                          </div>
                          <div>
                            <span className="text-slate-500 block text-[9px]">5Y High</span>
                            <span className="font-bold text-slate-200">₹{stock.five_year_high}</span>
                        ...
                        <div className="pt-2 border-t border-slate-900 text-[11px]">
                          <span className="text-rose-400 font-semibold">Primary Risk: </span>
                          <span className="text-slate-400">{stock.risk_factors[0]}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {report.neutral_candidates.map((stock: AgentStockInsight) => (
                      ...
                      >
                        <div>
                          <h4 className="font-bold text-slate-200 text-sm">{stock.symbol} ({stock.name})</h4>
                          <span className="text-[11px] text-slate-500">{stock.five_year_trend} • 5Y CAGR: {stock.five_year_cagr}%</span>
                        </div>
                        <span className="text-sm font-bold font-mono text-slate-300">{stock.probability_score}/100</span>
                      </div>
                    ))}
                  </div>
                </div>
