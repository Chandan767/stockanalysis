...
export const DashboardView: React.FC<DashboardViewProps> = (props: DashboardViewProps) => {
  ...
  if (error || !predictions) {
    return (
      <div className="p-8 text-center bg-red-900/20 border border-red-800/50 rounded-xl my-6">
        <AlertTriangle className="w-12 h-12 text-red-400 mx-auto mb-3" />
        <h3 className="text-xl font-bold text-red-300">Daily Prediction Engine Error</h3>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-900 via-indigo-950/80 to-slate-900 border border-indigo-500/20 p-6 md:p-8 shadow-2xl">
        <div className="absolute top-0 right-0 w-96 h-96 bg-indigo-500/10 rounded-full blur-3xl -mr-20 -mt-20 pointer-events-none" />
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <span className="px-3 py-1 bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 font-semibold text-xs rounded-full flex items-center gap-1.5 uppercase tracking-wider">
            ...
            <p className="text-slate-300 text-sm mt-2 max-w-2xl leading-relaxed">
              ...
              <span className="text-amber-400 font-semibold">NEUTRAL</span>) across 30+ Indian equities with calibrated probability scaling.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button
              ...
              className="px-5 py-3 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white font-bold text-sm rounded-xl shadow-lg shadow-indigo-600/30 hover:shadow-indigo-500/50 transition flex items-center gap-2"
            >
            ...
            <button
              onClick={loadData}
              className="p-3 bg-slate-800/80 hover:bg-slate-700 text-slate-300 hover:text-white rounded-xl border border-slate-700/60 transition"
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-8 pt-6 border-t border-slate-800">
          ...
          <div className="bg-slate-900/60 p-3.5 rounded-xl border border-slate-800">
            ...
            <div className="text-indigo-300 font-bold text-base mt-1 flex items-center gap-1.5">
              ...
              <div className="flex items-baseline gap-2">
                ...
                <span
                  className={`text-xs font-semibold ${
                    globals.signals.us_overnight_sp500_return >= 0 ? 'text-emerald-400' : 'text-rose-400'
                  }`}
                >
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 pb-4 border-b border-slate-800">
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold px-2.5 py-0.5 bg-indigo-500/20 text-indigo-300 rounded border border-indigo-500/30">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                ...
                <div className="bg-slate-800/60 p-3.5 rounded-xl border border-slate-700/60">
                  ...
                  <div
                    className={`font-extrabold text-xl mt-1 ${
                      backtestData.simulated_strategy_return_pct >= 0 ? 'text-emerald-400' : 'text-rose-400'
                    }`}
                  >
                ...
                <div className="bg-slate-800/60 p-3.5 rounded-xl border border-slate-700/60">
                  ...
                  <div className="text-slate-200 font-extrabold text-xl mt-1">
                ...
                <div className="bg-slate-800/60 p-3.5 rounded-xl border border-slate-700/60">
                  ...
                  <div className="text-indigo-400 font-extrabold text-xl mt-1">
                ...
                <div className="bg-slate-800/60 p-3.5 rounded-xl border border-slate-700/60">
                  ...
                  <div className="text-rose-400 font-extrabold text-xl mt-1">
                  ...
                  <tbody className="divide-y divide-slate-800">
                    {backtestData.daily_history.slice(-8).map((step, idx) => (
                      <tr key={idx} className="hover:bg-slate-800/40">
                        <td className="p-3 font-medium text-slate-200">{step.date}</td>
                        ...
                        <td className="p-3 font-bold">
                          <span
                            className={step.actual_return_pct >= 0 ? 'text-emerald-400' : 'text-rose-400'}
                          >
                            {step.actual_return_pct > 0 ? '+' : ''}
                            {step.actual_return_pct}%
                          </span>
                        </td>
                        <td className="p-3 font-semibold">{step.actual_direction}</td>
        ...
        return (
          ...
          >
            <div>
              <div className="flex items-start justify-between gap-2 mb-2">
                ...
                <div className="text-right">
                  ...
                  <div className="text-[11px] font-semibold text-slate-400">
                    ...
                    <span
                      className={
                        stock.expected_return_pct >= 0 ? 'text-emerald-400 font-bold' : 'text-rose-400 font-bold'
                      }
                    >
              ...
              {/* PROBABILITY BAR */}
              <div className="mt-3 bg-slate-900 p-3 rounded-xl border border-slate-800">
                <div className="flex justify-between text-xs font-medium mb-1.5">
                  <span className="text-slate-300 flex items-center gap-1">
                    {type === 'UP' && <TrendingUp className="w-3.5 h-3.5 text-emerald-400" />}
                    {type === 'DOWN' && <TrendingDown className="w-3.5 h-3.5 text-rose-400" />}
            ...
            <div className="mt-4 pt-3 border-t border-slate-700/50 flex items-center justify-between">
              <span className="text-[11px] text-slate-400">
                Confidence: <strong className="text-slate-200">{stock.confidence}</strong>
              </span>
              <button
                onClick={(e) => {
                  e.stopPropagation();
